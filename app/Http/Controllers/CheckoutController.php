        ...
        if ($request->payment_option != null) {
            ...
            if ($request->session()->get('order_id') != null) {
                if ($request->payment_option == 'paypal') {
                    ...
                    return $bkash->pay();
                }else if ($request->payment_option == 'walletmix') {
                    
                    $walletmix = new WalletmixController;
                    return $walletmix->pay();
...
//        if (Session::has('cart') && count(Session::get('cart')) > 0) {
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        if ($request->address_id == null) {
            flash(translate("Shipping address added"))->success();
            // flash(translate("Please add shipping address"))->success();
            return back();
        }
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            foreach ($carts as $key => $cartItem) {
                ...
                if(isset($cartItem['shipping_cost']) && is_array(json_decode($cartItem['shipping_cost'], true))) {
                    ...
                    foreach(json_decode($cartItem['shipping_cost'], true) as $shipping_region => $val) {
                        if($shipping_info['cost'] == $shipping_region) {
            ...
            $total = $subtotal + $tax + $shipping_info['cost'];
