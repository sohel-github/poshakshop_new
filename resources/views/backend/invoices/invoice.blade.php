            ...
            <table>
				...
				<tr>
					<td class="gry-color small">
						Address : {{ json_decode($order->shipping_address)->address }},<br/>
						Division : {{ json_decode($order->shipping_address)->division }},<br/>
						District : {{ json_decode($order->shipping_address)->district }},<br/>
						Area : {{ json_decode($order->shipping_address)->area }},<br/>
						Country : {{ json_decode($order->shipping_address)->country }}<br/>
			...
			<table class="padding text-left small border-bottom">
				<thead>
	                <tr class="gry-color" style="background: #eceff4;">
	                    ...
	                    <th width="15%" class="text-left">{{ translate('SKU') }}</th>
	                    <th width="15%" class="text-left">{{ translate('Attribute') }}</th>
				...
				<tbody class="strong">
	                @foreach ($order->orderDetails as $key => $orderDetail)
                        ...
                        {{-- ({{ $orderDetail->variation }}) --}}
							<tr class="">
								<td>{{ $orderDetail->product->name }} @if($orderDetail->variation != null) {{ \App\AttributeValue::where('id',$orderDetail->variation)->first()->attribute_value_name ?? '' }} @endif</td>
								<td>
					        ...
					        <table class="text-right sm-padding small strong">
						        <tbody>
							        ...
							        <tr>
							            ...
							            <td class="currency">৳{{ ($order->grand_total + json_decode($order->shipping_address)->cost) - $order->coupon_discount }}</td>
