...
<table class="table table-bordered aiz-table">
	...
	@foreach ($combinations as $key => $combination)
		...
		@if(strlen($str) > 0)
			...
			<tr class="variant">
				<td>
                    {{-- @dump(\App\AttributeValue::where('id',$str)->first()->attribute_value_name ?? '' , $str) --}}
                    <label for="" class="control-label">{{ \App\AttributeValue::where('id',$str)->first()->attribute_value_name ?? '' }}</label>
