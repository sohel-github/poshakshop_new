        ...
        <div class="row gutters-5">
            <div class="col text-center text-md-left">
                <address>
                    ...
                    <span class="w-50 fw-600">Email</span> : {{ json_decode($order->shipping_address)->email }}<br>
                    <span class="w-50 fw-600">Phone</span> : {{ json_decode($order->shipping_address)->phone }}<br>
                    ...
                    <span class="w-50 fw-600">Address</span> : {{ json_decode($order->shipping_address)->address }},<br/>
                    <span class="w-50 fw-600">Division</span> : {{ json_decode($order->shipping_address)->division }},<br/>
                    <span class="w-50 fw-600">District</span> : {{ json_decode($order->shipping_address)->district }},<br/>
                    <span class="w-50 fw-600">Area</span> : {{ json_decode($order->shipping_address)->area }},<br/>
                    <span class="w-50 fw-600">Country</span> : {{ json_decode($order->shipping_address)->country }}
                </address>
                ...
                <table class="table table-bordered aiz-table invoice-summary">
                    <thead>
                        <tr class="bg-trans-dark">
                            ...
                            <th data-breakpoints="lg" class="min-col text-center text-uppercase">{{translate('SKU')}}</th>
                            <th data-breakpoints="lg" class="min-col text-center text-uppercase">{{translate('Price')}}</th>
                            <th data-breakpoints="lg" class="min-col text-center text-uppercase">{{translate('Attribute')}}</th>
...
{{-- @dd(\App\AttributeValue::where('id',$orderDetail->variation)->first()) --}}
                            <td>
                                @if ($orderDetail->product != null)
                                ...
                                {{-- {{ \App\AttributeValue::where('id',$orderDetail->variation)->first()->attribute_value_name ?? '' }} --}}
                                @else
                            ...
                            <td>
                                {{-- @dd($orderDetail->product->stocks) --}}
                                @if ($orderDetail->product != null)
                               {{  $orderDetail->product->stocks->where('variant',$orderDetail->variation)->first()->sku ?? 'not found'  }}
                                    {{-- {{ $item->sku   .', '  ?? 'not found' }} --}}
                                    {{-- @break --}}
                                {{-- @endforeach --}}
                                @else
                            ...
                            <td class="text-center">
                                @if ($orderDetail->attribute_id != null)
                                 {{ \App\AttributeValue::where('id',$orderDetail->attribute_id)->first()->attribute_value_name ?? '' }}
                                 @else
                                <strong>{{ translate('N/A') }}</strong>
                                 @endif
                            </td>
            ...
            <table class="table">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ৳{{ json_decode($order->shipping_address)->cost }}
                    ...
                    <tr>
                        ...
                        <td class="text-muted h5">
                            ৳{{ ($order->grand_total + json_decode($order->shipping_address)->cost) - $order->coupon_discount }}
                            {{-- {{ single_price($order->grand_total) }} --}}
                        </td>
                    </tr>
                </tbody>
